...
The placement package is no longer available on CRAN and needs to be installed from Github as follows.
...
In this section we will look up some of the records with each of the three packages to show how easy it is. Purely from personal preference we will use `placement` for the rest of the work. Note that `placement` is no longer being actively maintained and the examples below are used for illustration. We now recommend switching to `ggmap` or `googleway` depending on your needs.
